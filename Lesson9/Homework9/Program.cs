...
//    secondName,
//    phoneNumber }
List<(string firstName, string lastName, string number)> records;
...
{
    records = new List<(string firstName, string lastName, string number)>();    
    try
    {
        if (records.Count == 0)  throw new Exception("No records in the file");
    }

    catch (Exception ex2)
    {
        records.Add(("Dmytro", "Bonislavskyi", "050-333-3333"));
        Console.WriteLine("Here is bussines exceptions log \n(legenda: wrong file name was cached and solved, then empty list of contacts was found and one entery was added)\n\n" );
        Console.WriteLine( 
...
void UpdateRecord(List<(string firstName, string lastName, string number)> records)
{
    ...
    if (rec.Count > 1)
    {
        ...
        {
            Console.WriteLine("That was unxcepteble input\n" + ex.Message.ToString());
        }
        finally
        {
            ShowPhoneBook(records);
...
List<(string firstName, string lastName, string number)> ReadFromFile()
{
    ...
    try
    {
        ...
        {
            throw new System.IO.FileNotFoundException("Wrong pass or file name exception");
        }
        //if else(File.)
        else {
            try
        ...
        //throw new System.Collections.IDictionary();
        //return new List<(string, string, string)>();
        //}
    ...
    {
        //Console.WriteLine(ex.Message);
        FileName = "PhoneBook.txt";//Correcting file name that was wrong
        records = new List<(string, string, string)>();
        SaveToFile(records);
        ReadFromFile();
        throw new Exception("Due to exception with file name, changes was made", ex);
        //return new List<(string, string, string)>();
    }
    catch (Exception ex)
    {
        Console.WriteLine("Another Exeption is " + ex.Message);
        throw;
        //return new List<(string, string, string)>();
    }
}